...
///
/// assert_eq!(to_binary(&k), "0000110".to_owned());
/// ```
pub fn from_binary(input: &[u8]) -> Term {
	let unspaced: Vec<u8> = input.iter().filter(|&b| ![9, 10, 13, 32].contains(b)).map(|b| *b).collect();

	_from_binary(&unspaced).unwrap().0
}
...
mod test {
	...
	#[test]
	fn variables() {
		assert_eq!(from_binary(b"10"), Var(1));
		assert_eq!(from_binary(b"110"), Var(2));
		assert_eq!(from_binary(b"1110"), Var(3));
	}

	#[test]
	fn abstractions() {
		assert_eq!(from_binary(b"0010"), abs(Var(1)));
		assert_eq!(from_binary(b"000010"), abs(abs(Var(1))));
		assert_eq!(from_binary(b"00000010"), abs(abs(abs((Var(1))))));
	}

	#[test]
	fn applications() {
		assert_eq!(from_binary(b"011010"), app(Var(1), Var(1)));
		assert_eq!(from_binary(b"0110110"), app(Var(1), Var(2)));
		assert_eq!(from_binary(b"0111010"), app(Var(2), Var(1)));
	}
	...
	fn parse_and_display() {
		...
		assert_eq!(format!("{}", from_binary(&*k)), "λλ2");
		assert_eq!(format!("{}", from_binary(&*v15)), "F");
		assert_eq!(format!("{}", from_binary(&*s)), "λλλ31(21)");
		assert_eq!(format!("{}", from_binary(&*succ)), "λλλ2(321)");
		assert_eq!(format!("{}", from_binary(&*quine)), "λ1((λ11)(λλλλλ14(3(55)2)))1");
		assert_eq!(format!("{}", from_binary(&*primes)), "λ(λ1(1((λ11)(λλλ1(λλ1)((λ441((λ11)(λ2(11))))(λλλλ13(2(64)))))(λλλ4(13)))))(λλ1(λλ2)2)");
		assert_eq!(format!("{}", from_binary(&*blc)), "(λ11)(λλλ1(λλλλ3(λ5(3(λ2(3(λλ3(λ123)))(4(λ4(λ31(21))))))(1(2(λ12))(λ4(λ4(λ2(14)))5))))(33)2)(λ1((λ11)(λ11)))");
	}
}
