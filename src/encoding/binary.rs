...
fn _to_bits(term: &Term, output: &mut Vec<u8>) {
    match term {
        ...
        App(boxed) => {
            ...
            output.append(&mut to_bits(&boxed.0));
            output.append(&mut to_bits(&boxed.1));
        }
    }
}
...
mod test {
    ...
    const QUINE: &[u8; 66] = b"000101100100011010000000000001011011110010111100111111011111011010";
